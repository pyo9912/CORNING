...
def train_KO_our_rag_generation(args, bert_model, tokenizer, train_dataset_raw, test_dataset_raw, train_knowledgeDB, all_knowledgeDB):
    ...
    if args.rag_our_model.upper() == 'C2DPR':
        ...
        logger.info("@@@@@Load Default RAG On Bert")
    ...
    #     rag_model.to('cpu')
    with torch.no_grad():
        epoch_play(args, rag_tokenizer, rag_model, test_dataloader, optimizer, scheduler, 0, faiss_dataset, all_knowledgeDB=all_knowledgeDB, our_best_model=our_best_model, knowledge_index_rerank=knowledge_index_rerank, mode='knowledgeCheck')
    ...
    for epoch in range(args.rag_epochs):
        ...
        hitDic, hitdic_ratio, output_str = epoch_play(args, rag_tokenizer, rag_model, train_dataloader, optimizer, scheduler, epoch, faiss_dataset, all_knowledgeDB=all_knowledgeDB, our_best_model=our_best_model, knowledge_index_rerank=knowledge_index_rerank, mode='train')

        rag_model.eval()
        with torch.no_grad():
            hitDic, hitdic_ratio, output_str = epoch_play(args, rag_tokenizer, rag_model, test_dataloader, optimizer, scheduler, epoch, faiss_dataset, all_knowledgeDB=all_knowledgeDB, our_best_model=our_best_model, knowledge_index_rerank=knowledge_index_rerank, mode='test')
...
def epoch_play(args, tokenizer, model, data_loader, optimizer, scheduler, epoch, faiss_dataset, all_knowledgeDB=None, our_best_model=None, knowledge_index_rerank=None, mode='train'):
    from tqdm import tqdm
    # data_loader
    ...
    for batch in tqdm(data_loader, desc=f"Epoch {epoch}__{mode}", bar_format=' {l_bar} | {bar:23} {r_bar}'):
        ...
        if our_best_model is not None:
            batch_size = source_ids.size(0)
            batch_input_text = tokenizer.generator(tokenizer.question_encoder.batch_decode(source_ids, skip_special_tokens=True)).input_ids
            # batch_input_text = tokenizer.question_encoder.batch_decode(source_ids, skip_special_tokens=True).input_ids
            ...
            doc_scores = torch.topk(doc_all_scores, k=5).values
            context_input_ids = []
            for bid in range(batch_size):
                context_input_id = batch_input_text[bid][-200:]
                retrieved_docs = tokenizer.generator([all_knowledgeDB[did] for did in retrieved_doc_ids[bid]]).input_ids
                for r_idx, rdoc in enumerate(retrieved_docs):
                    retrieved_docs[r_idx] = retrieved_docs[r_idx][:100]
                    retrieved_docs[r_idx].extend(context_input_id)
                    retrieved_docs[r_idx] = retrieved_docs[r_idx][:300]
                    retrieved_docs[r_idx] = retrieved_docs[r_idx] + [tokenizer.generator.pad_token_id] * (300 - len(retrieved_docs[r_idx]))
                context_input_ids.extend(retrieved_docs)
            context_input_ids = torch.tensor(context_input_ids)
            context_attention_mask = torch.ne(context_input_ids, tokenizer.generator.pad_token_id)

            outputs = model(
                context_input_ids=context_input_ids.to(args.device),
                context_attention_mask=context_attention_mask.to(args.device),
                doc_scores=doc_scores,
                output_retrieved=True,
                labels=target_ids,
            )
            retrieved_docs_pt = retrieved_doc_ids
        else:
        ...
        # batch_top5_docs = [all_knowledgeDB[i] for i in retrieved_docs_pt]
        batch_top5_docs = []
        for docs in retrieved_docs_pt:
            batch_top5_docs.append([all_knowledgeDB[did] for did in docs])
...
def KOrag_model_weight_logging(args, rag_model, epoch, mode, faiss_dataset):
    def getweight(bert):
        return bert.layer[0].attention.self.key.weight[0][:50][0]
